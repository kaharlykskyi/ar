...
$(document).ready(function () {
    ...
    $().UItoTop({easingType: 'easeOutQuart'}); // Ui To Top

    var fixnav = $('.bg-menu'); // fixed mainmenu

    $(function () {
        $('.view-source .hide-text').hide();
        $a = $('.view-source a');
    ...
    $(window).scroll(function () {
        if ($(this).scrollTop() > 300) {
            fixnav.addClass("f-nav");
        } else {
            fixnav.removeClass("f-nav");
        }
    });
...
function display(view) {
    if (view == 'list') {
        ...
        $('.product_list > li:visible').removeAttr('class').addClass('ajax_block_product col-xs-12');
        $('.product_list > li:visible').each(function (index, element) {
            var html = '';
            html += '<div class="product-container"><div class="row">';
            html += '<div class="left-block col-xs-4 col-sm-5">' + $(element).find('.left-block').html() + '</div>';
            html += '<div class="center-block col-xs-8 col-sm-7">';
        ...
        $('.product_list > li:visible').each(function (index, element) {
            if (nbItemsPerLine && nbItemsPerLineTablet && nbItemsPerLineMobile) {
                $(element).removeAttr('class').addClass('ajax_block_product col-xs-' + 12 / nbItemsPerLineMobile + ' col-sm-' + 12 / nbItemsPerLineTablet + ' col-md-' + 12 / nbItemsPerLine);
...
function accordion(status) {
    if (status == 'enable') {
        $('#product .product-information .tab-content > h3, #right_column .block:not(#layered_block_left) .title_block, #left_column .block:not(#layered_block_left) .title_block, #left_column #newsletter_block_left h4').on('click', function () {
            ...
            $(this).next('.tab-pane').stop().slideToggle('medium');
        });
        ...
        $('#product .product-information .tab-content > h3, #right_column .block:not(#layered_block_left) .title_block, #left_column .block:not(#layered_block_left) .title_block, #left_column #newsletter_block_left h4').removeClass('active').off().parent().find('.block_content, .tab-pane').removeAttr('style').not('.tab-pane').slideDown('fast');
...
$(window).load(function () {
    $("#header-login #header-login-content > ul li").each(function () {
        if ($(this).hasClass("login")) {
            $(this).parent().parent().addClass('active-login')
        }
    })
    $("#header-login .header-login-content").each(function () {
        if ($(this).hasClass("is-logged")) {
